...
Status: Core 1H first evidence slice
...
The same baseline records structural allocation and trap sites, owned bindings,
maximum live-owned pressure, and exact total-function counts. These are
separate measurements; this slice does not infer allocation volume, peak bytes,
or a complete execution-step bound.

## Boundary

Source-level resource contracts remain unapproved. They require application
evidence showing which assumptions inference cannot recover, followed by a
separate weighted feature decision, diagnostics, semantics, and permanent
tests.
