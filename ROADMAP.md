...
portable reference and all optimizations consume Core 1D's checked artifact or
preserve one canonical runtime operation.

Planned work is ordered by measured evidence:

1. Make small checked runtime operations visible to ordinary C optimization,
   preserving identical cold trap and allocation paths.
2. Profile the remaining real-program gaps by operation and generated shape;
   do not infer optimization work from aggregate runtime alone.
3. Add bounded scalar range or control facts only where they can prove a
   specific overflow or bounds check redundant. Unknown facts keep the check.
4. Specialize generic collection access only from exact checked element types
   and retained storage plans, without changing layout or ownership.
5. Improve region precision and stack placement only when escape/liveness proof
   can select a narrower lifetime; never add source lifetime annotations merely
   for performance.
6. Keep external C compilation, compiler latency, generated bytes, binary size,
   runtime, traps, sanitizers, and allocation failure as separate gates.

The first slice adopts D0059's inlinable checked runtime. Arithmetic, byte, and
vector hot paths have one header-visible implementation, while slow allocation,
I/O, region, and trap behavior stays out-of-line. Same-host measurements improve
sieve 3.02x, matrix 4.14x, byte frequency 6.11x, and knapsack 1.80x. Eight new
trap fixtures bring the corpus to 109 and pin every moved failure family;
native runtime-ratio budgets are tightened rather than merely recording the
gain. Five warm self-checks consume 0.05 seconds of user CPU each, improving the
